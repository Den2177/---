...
cnv.numberConverterCommaSeparatedDigit = function(parseTree) {
    return parseInt(parseTree.text.replace(/(\.|\s)/g,''));
};
...
cnv.numberDucklingConverter = function(pt) {
    if (!pt || !pt.value || !pt.value.value) {
        return "Not a number";
    }
    return parseInt(pt.value.value);
};
$global.$converters = $global.$converters || {};
var cnv = $global.$converters;
...
cnv.PhoneNumberSameDigits = function(pt) {
    ...
    var number = pt.oneToNineNormForm1 ? pt.oneToNineGent1[0].value : 0;
    var output = " ";
